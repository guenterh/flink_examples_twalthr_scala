...
public class Example_06_DataStream_Join {
    ...
    //ungefähr ab Minute 22

  public static void main(String[] args) throws Exception {
    StreamExecutionEnvironment env = StreamExecutionEnvironment.getExecutionEnvironment();

    // switch to batch mode on demand
    /* run the datastream API in batch mode
    it's not super special because it's just a single flag
    but this flag executes under the hood a lot of components (it chages the behaviour of a lot of components)
    to make a streaming DataStream API pipeline to run very efficient in batch mode
     */
    ...
    KafkaSource<Transaction> transactionSource =
        KafkaSource.<Transaction>builder()
            ...
            /*
            make our sources bounded so that they are not continuously evaluating data but and after the
            latest offset
            wir müssen die pipeline nicht ändern - bleibt genau gleich
            but now we are able to run this streaming pipeline in batch mode on bounded data
            wenn wir nun mehr Daten hätten, würden wir die performance Verbesserung feststellen wenn wir historical data
            verarbeiten würden
             */
    ...
    customerStream
        ...
        .process(
            new KeyedCoProcessFunction<Long, Customer, Transaction, String>() {
                ...
                /*
                because we want to join we might have to buffer both sides
                because one stream can be quicker as the other side
                 */
